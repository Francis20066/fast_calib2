                    ...
                    const std::string& bag_path)
{
    ...
    if (ret != 0)
    {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("lidar_center_test"),
                            "[LiDAR Test] Failed to save debug cloud to " << output_path);
        return false;
    }
                           ...
                           const std::string& bag_path)
{
    ...
    {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("lidar_center_test"),
                            "[LiDAR Test] Failed to save center coordinates to " << output_path);
        return false;
    }
...
int main(int argc, char** argv)
{
    ...
    auto node = std::make_shared<rclcpp::Node>("lidar_center_test");
    ...
    if (!loadCloudFromBag(node->get_logger(), params.bag_path, params.lidar_topic, cloud, detected_type))
    {
        return 1;
    }
    ...
    sortPatternCenters(raw_centers, centers, "lidar", params.lidar_flip_yz);
