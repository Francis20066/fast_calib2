...
private:
                                              ...
                                              float& threshold) const
    {
        ...
        if (label == "Annulus")
        {
            if (otsu_threshold > high_quantile * 1.05f)
            {
                RCLCPP_WARN(logger_, "[LiDAR] %s Otsu %.3f >> p92 %.3f; use p92 as threshold.",
                         label.c_str(), otsu_threshold, high_quantile);
                threshold = high_quantile;
            }
            else
            {
                threshold = std::min(threshold, std::max(otsu_threshold, high_quantile));
            }
            ...
            if (saturated_foreground)
            {
                ...
                RCLCPP_WARN(logger_, "[LiDAR] %s threshold %.3f is locked by saturated returns; fallback to %.3f.",
        ...
        {
            RCLCPP_WARN(logger_, "[LiDAR] %s threshold %.3f is dominated by saturated outliers; fallback to p92 %.3f.",
                                    ...
                                    const std::string& label) const
    {
        ...
        {
            RCLCPP_WARN(logger_, "[LiDAR] Unable to compute high-intensity threshold for %s.", label.c_str());
            return false;
        }
        ...
        RCLCPP_INFO(logger_, "[LiDAR] %s intensity range: %.3f - %.3f", label.c_str(), min_i, max_i);
        RCLCPP_INFO(logger_, "[LiDAR] %s Otsu: %.3f, foreground p15: %.3f, p92: %.3f, relative high: %.3f, threshold: %.3f",
                 label.c_str(), otsu_threshold, foreground_low, high_quantile, relative_high, threshold);
        RCLCPP_INFO(logger_, "[LiDAR] %s high-intensity points: %zu", label.c_str(), output->size());
        return !output->empty();
    }
                                             ...
                                             const std::string& label) const
    {
        ...
        {
            RCLCPP_WARN(logger_, "[LiDAR] Empty input cloud, cannot extract high-intensity points for %s.", label.c_str());
            return false;
        }
        if (!plane_coefficients || plane_coefficients->values.size() < 4)
        {
            RCLCPP_WARN(logger_, "[LiDAR] Invalid plane coefficients for %s.", label.c_str());
            return false;
        }
        ...
        bool interpolate_boundary) const
    {
        ...
        if (!input || input->empty())
        {
            RCLCPP_WARN(logger_, "[LiDAR] Empty input cloud, cannot extract ring intensity boundaries for %s.", label.c_str());
            return false;
        }
        if (!plane_coefficients || plane_coefficients->values.size() < 4)
        {
            RCLCPP_WARN(logger_, "[LiDAR] Invalid plane coefficients for %s ring boundary extraction.", label.c_str());
            return false;
        }
        ...
        if (!computeHighIntensityThreshold(intensities, threshold, min_i, max_i,
                                           ...
                                           high_quantile, relative_high))
        {
            RCLCPP_WARN(logger_, "[LiDAR] Unable to compute high-intensity threshold for %s ring boundaries.", label.c_str());
            return false;
        }
        adjustHighIntensityThresholdForLabel(label, min_i, max_i, otsu_threshold,
                                             foreground_low, high_quantile,
        ...
        RCLCPP_INFO(logger_, "[LiDAR] %s ring-boundary intensity range: %.3f - %.3f", label.c_str(), min_i, max_i);
        RCLCPP_INFO(logger_, "[LiDAR] %s ring-boundary Otsu: %.3f, foreground p15: %.3f, p92: %.3f, relative high: %.3f, threshold: %.3f, jump threshold: %.3f",
                 label.c_str(), otsu_threshold, foreground_low, high_quantile,
                 relative_high, threshold, jump_threshold);
        RCLCPP_INFO(logger_, "[LiDAR] %s ring-boundary rings: %zu, valid neighbors: %zu, transitions: %zu, boundary points: %zu",
                 label.c_str(), indices_by_ring.size(), valid_neighbor_count,
                 transition_count, output->size());
        RCLCPP_INFO(logger_, "[LiDAR] %s ring-boundary point mode: %s",
                            ...
                            BoardFrame& frame) const
    {
        ...
        if (normal.norm() < 1e-6)
        {
            RCLCPP_WARN(logger_, "[LiDAR] Auto ROI failed: invalid rough plane normal.");
            return false;
        }
        normal.normalize();
        ...
        {
            RCLCPP_WARN(logger_, "[LiDAR] Auto ROI failed: cannot build board axes.");
            return false;
        }
        ...
        if (x_axis.norm() < 1e-6)
        {
            RCLCPP_WARN(logger_, "[LiDAR] Auto ROI failed: invalid board x-axis.");
            return false;
        }
        x_axis.normalize();
        ...
        if (y_axis.norm() < 1e-6)
        {
            RCLCPP_WARN(logger_, "[LiDAR] Auto ROI failed: invalid board y-axis.");
            return false;
        }
        y_axis.normalize();
                             ...
                             pcl::PointCloud<Common::Point>::Ptr board_roi_cloud) const
    {
        ...
        pcl::PointCloud<Common::Point>::Ptr manual_filtered(new pcl::PointCloud<Common::Point>);
        manualPassThroughFilter(cloud, manual_filtered);
        if (manual_filtered->size() >= 1000)
        {
            copyFiniteRangePoints(manual_filtered, finite_cloud);
            RCLCPP_INFO(logger_, "[LiDAR] Auto ROI manual pre-filter cloud size: %zu", finite_cloud->size());
        }
        else
        {
            copyFiniteRangePoints(cloud, finite_cloud);
            RCLCPP_WARN(logger_, "[LiDAR] Auto ROI manual pre-filter too small (%zu), use full cloud.",
                     manual_filtered->size());
        }
        RCLCPP_INFO(logger_, "[LiDAR] Auto ROI finite cloud size: %zu", finite_cloud->size());
        if (finite_cloud->size() < 1000)
        {
            RCLCPP_WARN(logger_, "[LiDAR] Auto ROI failed: too few finite points.");
            return false;
        }
        ...
        RCLCPP_INFO(logger_, "[LiDAR] Auto ROI max-intensity voxel cloud size: %zu (leaf %.3f m)",
                 coarse_cloud->size(), auto_roi_voxel_leaf_);
        if (coarse_cloud->size() < 1000)
        {
            RCLCPP_WARN(logger_, "[LiDAR] Auto ROI failed: too few points after max-intensity voxel sampling.");
            return false;
        }
        ...
        {
            RCLCPP_WARN(logger_, "[LiDAR] Auto ROI failed: too few high-intensity points.");
            return false;
        }
        ...
        if (cluster_indices.size() > 20)
        {
            ...
            ec.extract(cluster_indices);
            RCLCPP_WARN(logger_, "[LiDAR] Auto ROI too many clusters, retry with min size 500: %zu",
                     cluster_indices.size());
        }

        RCLCPP_INFO(logger_, "[LiDAR] Auto ROI high-intensity clusters: %zu", cluster_indices.size());
        if (cluster_indices.size() < TARGET_NUM_CIRCLES)
        {
            RCLCPP_WARN(logger_, "[LiDAR] Auto ROI failed: fewer than 4 high-intensity clusters.");
            return false;
        }
        ...
        if (!selectGeometryConsistentCentersByDistances(candidate_centers, selected_indices, 0.25))
        {
            RCLCPP_WARN(logger_, "[LiDAR] Auto ROI failed: no high-intensity cluster set matches target geometry.");
            return false;
        }
        ...
        {
            RCLCPP_WARN(logger_, "[LiDAR] Auto ROI failed: cannot fit rough plane from selected reflective annuli.");
            return false;
        }
        ...
        RCLCPP_INFO(logger_, "[LiDAR] Auto ROI board cloud size: %zu", board_roi_cloud->size());
        if (board_roi_cloud->size() < 1000)
        {
            RCLCPP_WARN(logger_, "[LiDAR] Auto ROI failed: board ROI is too small.");
            return false;
        }
                         ...
                         pcl::PointCloud<Common::Point>::Ptr board_roi_cloud) const
    {
        ...
        {
            RCLCPP_INFO(logger_, "[LiDAR] Using automatic board ROI extraction.");
            if (!extractAutoBoardRoi(cloud, board_roi_cloud))
            {
                RCLCPP_WARN(logger_, "[LiDAR] Automatic board ROI extraction failed, fallback to manual pass-through.");
                manualPassThroughFilter(cloud, board_roi_cloud);
                if (board_roi_cloud->size() < 1000)
                {
                    RCLCPP_WARN(logger_, "[LiDAR] Manual board ROI cloud too small: %zu", board_roi_cloud->size());
                    return false;
                }
            }
        }
        ...
        {
            RCLCPP_INFO(logger_, "[LiDAR] Using manual pass-through board ROI extraction.");
            manualPassThroughFilter(cloud, board_roi_cloud);
        }
        return true;
    }
                              ...
                              pcl::ModelCoefficients::Ptr plane_coefficients)
    {
        *filtered_cloud_ = *board_roi_cloud;
        RCLCPP_INFO(logger_, "[LiDAR] Board ROI cloud size: %zu", filtered_cloud_->size());
        if (filtered_cloud_->size() < 1000)
        {
            RCLCPP_WARN(logger_, "[LiDAR] Too few board ROI points.");
            return false;
        }

        if (!fitPlaneRansac(filtered_cloud_, plane_coefficients, 0.01))
        {
            RCLCPP_WARN(logger_, "[LiDAR] Unable to fit board plane from ROI cloud.");
            return false;
        }
        ...
        {
            RCLCPP_WARN(logger_, "[LiDAR] Too few board plane inliers.");
            return false;
        }
        return true;
    }
                             ...
                             PlaneAlignment& alignment)
    {
        ...
        if (normal.norm() < 1e-6)
        {
            RCLCPP_WARN(logger_, "[LiDAR] Invalid plane normal.");
            return false;
        }
        normal.normalize();
        ...
        if (cnt == 0)
        {
            RCLCPP_WARN(logger_, "[LiDAR] No board plane points to align.");
            return false;
        }
                                                 ...
                                                 const PlaneAlignment& alignment)
    {
        ...
        RCLCPP_INFO(logger_, "Extracted %zu high-intensity annulus points before voxel sampling.", edge_cloud_->size());

        if (edge_cloud_->size() < 20)
        {
            RCLCPP_WARN(logger_, "[LiDAR] Too few annulus points after intensity extraction.");
            return false;
        }
        ...
        if (annulus_cloud_downsampled->size() < edge_cloud_->size() / 8 &&
            edge_cloud_->size() > annulus_cloud_downsampled->size() * 4)
        {
            RCLCPP_WARN(logger_, "[LiDAR] Annulus voxel sampling too aggressive; keep raw annulus points.");
        }
        else
        {
            edge_cloud_.swap(annulus_cloud_downsampled);
        }
        RCLCPP_INFO(logger_, "[LiDAR] Annulus points after closest-to-centroid voxel sampling: %zu (leaf %.3f m)",
                                    ...
                                    bool interpolate_boundary = true)
    {
        ...
        {
            RCLCPP_WARN(logger_, "[LiDAR] Ring-boundary annulus extraction failed; fallback to high-intensity annulus points.");
            if (!extractHighIntensityPointsNearPlane(plane_cloud_, plane_coefficients,
                                                     plane_distance_threshold,
                                                     annulus_original_cloud_, "Annulus"))
            {
        ...
        {
            RCLCPP_WARN(logger_, "[LiDAR] Ring-boundary annulus points are too sparse (%zu); fallback to high-intensity annulus points.",
        ...
        RCLCPP_INFO(logger_, "[LiDAR] Extracted %zu high-intensity annulus points.",
                 annulus_original_cloud_->size());
        ...
        {
            RCLCPP_WARN(logger_, "[LiDAR] Voxel sampling made annulus cloud too sparse; keep raw annulus points for fitting.");
        }
        else
        {
            edge_cloud_.swap(annulus_cloud_downsampled);
    ...
    bool selectBestGeometryConsistentCenters(const pcl::PointCloud<pcl::PointXYZ>::Ptr& candidates,
                                               std::vector<int>& selected_indices) const
    {
        if (selectGeometryConsistentCenters(candidates, selected_indices))
        {
            return true;
        }
        if (selectGeometryConsistentCentersByDistances(candidates, selected_indices, GEOMETRY_TOLERANCE))
        {
            RCLCPP_WARN(logger_, "[LiDAR] Square geometry check failed; fallback to distance-based selection.");
            return true;
        }
        return false;
    }
    ...
    void clusterAnnulusCloud(std::vector<pcl::PointIndices>& cluster_indices) const
    {
        ...
        const std::vector<std::pair<double, int>> configs = {
            {0.012, 80}, {0.018, 100}, {0.025, 120}, {0.032, 150},
        };

        double best_score = -1.0;
        std::vector<pcl::PointIndices> best_indices;
        ...
        {
            std::vector<pcl::PointIndices> trial_indices;
            ec.setClusterTolerance(cfg.first);
            ec.setMinClusterSize(cfg.second);
            ec.extract(trial_indices);
            if (trial_indices.empty()) continue;

            const double n = static_cast<double>(trial_indices.size());
            double score = 0.0;
            if (n >= 4.0 && n <= 12.0)
            {
                score = 100.0 - std::fabs(n - 4.0) * 5.0;
            }
            else if (n > 12.0)
            {
                score = 40.0 - (n - 12.0);
            ...
            {
                best_score = score;
                best_indices = std::move(trial_indices);
            }
        }

        cluster_indices = std::move(best_indices);
    ...
    void clusterMechanicalAnnulusBoundaryCloud(std::vector<pcl::PointIndices>& cluster_indices) const
    {
        ...
        RCLCPP_INFO(logger_, "[LiDAR] Mechanical annulus boundary clusters: %zu", cluster_indices.size());
    }

    bool tryAppendAnnulusCenterFromCluster(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cluster,
                                           pcl::PointCloud<pcl::PointXYZ>::Ptr candidate_centers,
                                           ...
                                           bool allow_split) const
    {
        ...
        {
            RCLCPP_WARN(logger_, "[LiDAR] Annulus fit failed for cluster %zu with %zu points.",
                     cluster_label, cluster->size());
            return false;
        }

        const bool radius_ok = std::fabs(fit.radius - circle_radius_) < 0.08;
        const bool error_ok = fit.mean_abs_error < 0.045;
        RCLCPP_INFO(logger_, "[LiDAR] Annulus cluster %zu: points=%zu, center=(%.4f, %.4f), radius=%.4f, mean abs error=%.4f",
                 cluster_label, cluster->size(), fit.x, fit.y, fit.radius, fit.mean_abs_error);

        if (radius_ok && error_ok)
        {
            candidate_centers->push_back(pcl::PointXYZ(static_cast<float>(fit.x),
                                                         static_cast<float>(fit.y),
                                                         0.0f));
            return true;
        }
        ...
        if (cluster->size() >= 60)
        {
            ConcentricCircleFitResult concentric_fit;
            ...
            if (fitFixedRadiiConcentricCenterRobust(cluster, inner_radius, outer_radius, concentric_fit) &&
                concentric_fit.rmse < 0.04 && concentric_fit.support >= 30)
            {
                RCLCPP_INFO(logger_, "[LiDAR] Cluster %zu fallback concentric center=(%.4f, %.4f), rmse=%.4f",
                         cluster_label, concentric_fit.x, concentric_fit.y, concentric_fit.rmse);
                candidate_centers->push_back(pcl::PointXYZ(static_cast<float>(concentric_fit.x),
                                                             static_cast<float>(concentric_fit.y),
                                                             0.0f));
                return true;
            }
        }

        if (allow_split && !radius_ok && fit.radius > circle_radius_ + 0.08 && cluster->size() >= 160)
        {
            pcl::search::KdTree<pcl::PointXYZ>::Ptr tree(new pcl::search::KdTree<pcl::PointXYZ>);
            ...
            std::vector<pcl::PointIndices> subclusters;
            ec.extract(subclusters);
            if (subclusters.size() > 1)
            {
                RCLCPP_WARN(logger_, "[LiDAR] Split oversized cluster %zu into %zu subclusters.",
                         cluster_label, subclusters.size());
                bool added = false;
                for (size_t j = 0; j < subclusters.size(); ++j)
                {
                    pcl::PointCloud<pcl::PointXYZ>::Ptr subcluster(new pcl::PointCloud<pcl::PointXYZ>);
                    for (const auto& idx : subclusters[j].indices)
                    {
                        subcluster->push_back(cluster->points[idx]);
                    }
                    added |= tryAppendAnnulusCenterFromCluster(subcluster, candidate_centers,
                                                               cluster_label * 100 + j, false);
                }
                return added;
            }
        }

        if (allow_split && radius_ok && fit.radius < circle_radius_ - 0.02 && cluster->size() >= 100)
        {
            pcl::search::KdTree<pcl::PointXYZ>::Ptr tree(new pcl::search::KdTree<pcl::PointXYZ>);
            tree->setInputCloud(cluster);
            pcl::EuclideanClusterExtraction<pcl::PointXYZ> ec;
            ec.setClusterTolerance(0.006);
            ...
            if (subclusters.size() > 1)
            {
                RCLCPP_WARN(logger_, "[LiDAR] Split undersized-radius cluster %zu into %zu subclusters.",
                         cluster_label, subclusters.size());
                bool added = false;
                for (size_t j = 0; j < subclusters.size(); ++j)
                {
        ...
        RCLCPP_WARN(logger_, "[LiDAR] Reject cluster %zu: radius_ok=%d, error_ok=%d.",
                 cluster_label, radius_ok, error_ok);
        return false;
    }
                                       ...
                                       pcl::PointCloud<pcl::PointXYZ>::Ptr candidate_centers) const
    {
        ...
        for (size_t i = 0; i < cluster_indices.size(); ++i)
        {
            ...
            tryAppendAnnulusCenterFromCluster(cluster, candidate_centers, i, true);
        }
    }
                                                 ...
                                                 pcl::PointCloud<pcl::PointXYZ>::Ptr candidate_centers) const
    {
        ...
        for (size_t i = 0; i < cluster_indices.size(); ++i)
        {
            ...
            {
                RCLCPP_WARN(logger_, "[LiDAR] Concentric annulus fit failed for cluster %zu with %zu boundary points.",
                         i, cluster->size());
                continue;
            }

            const bool error_ok = fit.rmse < 0.04;
            const bool width_ok = (fit.outer_radius - fit.inner_radius) > 0.006 &&
                                  (fit.outer_radius - fit.inner_radius) < 0.10;
            RCLCPP_INFO(logger_, "[LiDAR] Concentric annulus cluster %zu: points=%zu, support=%d, center=(%.4f, %.4f), radii=(%.4f, %.4f), mean abs=%.4f, rmse=%.4f",
            ...
            {
                RCLCPP_WARN(logger_, "[LiDAR] Reject concentric cluster %zu: error_ok=%d, width_ok=%d.",
                         i, error_ok, width_ok);
                continue;
            }
        ...
        RCLCPP_INFO(logger_, "[LiDAR] Concentric annulus center candidates: %zu", candidate_centers->size());
    }
        ...
        pcl::PointCloud<pcl::PointXYZ>::Ptr boundary_edge_cloud) const
    {
        ...
        for (size_t i = 0; i < cluster_indices.size(); ++i)
        {
            ...
            {
                RCLCPP_WARN(logger_, "[LiDAR] Solid boundary extraction failed for cluster %zu with %zu points.",
                         i, cluster->size());
                continue;
            }
            ...
            {
                RCLCPP_WARN(logger_, "[LiDAR] Solid boundary concentric fit failed for cluster %zu with %zu boundary points.",
                         i, boundary_cloud->size());
                continue;
            }

            const bool error_ok = fit.rmse < 0.04;
            RCLCPP_INFO(logger_, "[LiDAR] Solid boundary cluster %zu: cluster=%zu, boundary=%zu, center=(%.4f, %.4f), radii=(%.4f, %.4f), mean abs=%.4f, rmse=%.4f",
                     i, cluster->size(), boundary_cloud->size(), fit.x, fit.y,
                     fit.inner_radius, fit.outer_radius, fit.mean_abs_error, fit.rmse);
            if (!error_ok)
            {
                RCLCPP_WARN(logger_, "[LiDAR] Reject solid boundary cluster %zu: rmse %.4f.", i, fit.rmse);
                continue;
            }
        ...
        RCLCPP_INFO(logger_, "[LiDAR] Solid boundary concentric center candidates: %zu", candidate_centers->size());
    }
...
public:
    ...
    void detect_mech_lidar(pcl::PointCloud<Common::Point>::Ptr cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr center_cloud)
    {
        ...
        auto run_mechanical_fit = [&](bool interpolate_boundary) -> MechanicalFitAttempt
        {
            ...
            if (!selectGeometryConsistentCentersByDistances(attempt.candidate_centers,
                                                            ...
                                                            0.035))
            {
                RCLCPP_WARN(logger_, "[LiDAR] Mechanical concentric fitting (%s boundary) did not produce 4 geometry-consistent centers from %zu candidates.",
            ...
            *(attempt.edge_cloud) = *edge_cloud_;
            ...
            RCLCPP_INFO(logger_, "[LiDAR] Mechanical concentric fitting selected %s ring-boundary points.",
                     interpolate_boundary ? "interpolated" : "high-side");
            RCLCPP_INFO(logger_, "[LiDAR] Mechanical %s boundary geometry max error %.2f mm.",
                     interpolate_boundary ? "interpolated" : "high-side",
        ...
        *edge_cloud_ = *(best_attempt->edge_cloud);
        RCLCPP_INFO(logger_, "[LiDAR] Mechanical concentric centers selected with geometry max error %.2f mm.",
    ...
    void detect_solid_lidar(pcl::PointCloud<Common::Point>::Ptr cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr center_cloud)
    {
        ...
        auto evaluateAttempt = [&](SolidFitAttempt& attempt) {
            attempt.selected_indices.clear();
            attempt.geometry_error = std::numeric_limits<double>::max();
            attempt.success = false;
            if (!attempt.candidates || attempt.candidates->size() < TARGET_NUM_CIRCLES)
            {
                return;
            }
            if (!selectBestGeometryConsistentCenters(attempt.candidates, attempt.selected_indices))
            {
                return;
            }
            attempt.geometry_error =
                selectedCentersGeometryMaxError(attempt.candidates, attempt.selected_indices);
            attempt.success = true;
        };

        std::vector<SolidFitAttempt> attempts(3);
        attempts[0].label = "circle";
        attempts[0].candidates.reset(new pcl::PointCloud<pcl::PointXYZ>);
        attempts[1].label = "concentric";
        attempts[1].candidates.reset(new pcl::PointCloud<pcl::PointXYZ>);
        attempts[2].label = "boundary";
        attempts[2].candidates.reset(new pcl::PointCloud<pcl::PointXYZ>);
        attempts[2].boundary_cloud.reset(new pcl::PointCloud<pcl::PointXYZ>);

        fitAnnulusCentersFromClusters(cluster_indices, attempts[0].candidates);
        evaluateAttempt(attempts[0]);

        fitConcentricAnnulusCentersFromClusters(cluster_indices, attempts[1].candidates);
        evaluateAttempt(attempts[1]);

        fitSolidBoundaryConcentricCentersFromClusters(cluster_indices,
                                                      attempts[2].candidates,
                                                      attempts[2].boundary_cloud);
        evaluateAttempt(attempts[2]);

        const SolidFitAttempt* best_attempt = nullptr;
        for (const auto& attempt : attempts)
        {
            if (!attempt.success) continue;
            if (!best_attempt || attempt.geometry_error < best_attempt->geometry_error)
            {
        ...
        if (!best_attempt)
        {
            RCLCPP_WARN(logger_, "[LiDAR] Unable to select 4 geometry-consistent annulus centers from all solid fitting paths.");
            return;
        }

        RCLCPP_INFO(logger_, "[LiDAR] Selected solid annulus fit path '%s' with geometry max error %.2f mm.",
                 best_attempt->label.c_str(), best_attempt->geometry_error * 1000.0);

        if (best_attempt->boundary_cloud && !best_attempt->boundary_cloud->empty())
        {
            transformAlignedPointsBackToLidar(best_attempt->boundary_cloud, alignment, boundary_original_cloud_);
        }

        transformCentersBackToLidar(best_attempt->candidates,
                                    best_attempt->selected_indices,
                                    alignment,
                                    center_cloud);
    }
