  ...
  public:
    ...
    {      
      ...
      if (ids.size() >= min_detected_markers_ && ids.size() <= TARGET_NUM_CIRCLES) 
      {
        ...
        for (const auto& p : markerObjPoints)
          objPoints.emplace_back(p.x * marker_size_, p.y * marker_size_, 0);
        for (size_t i = 0; i < corners.size(); ++i) {
          cv::Vec3d r, t;
          cv::solvePnP(objPoints, corners[i], cameraMatrix_, distCoeffs_, r, t);
          rvecs.push_back(r);
          tvecs.push_back(t);
        }
