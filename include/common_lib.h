...
Params loadParameters(const rclcpp::Node::SharedPtr& node) {
  ...
  node->declare_parameter("annulus_half_width", 0.025);
  node->declare_parameter("board_width", 1.4);
  node->declare_parameter("board_height", 1.0);
  node->declare_parameter("board_roi_margin", 0.08);
  node->declare_parameter("board_roi_depth", 0.12);
  node->declare_parameter("auto_roi_voxel_leaf", 0.01);
  node->declare_parameter("annulus_voxel_leaf", 0.005);
  node->declare_parameter("image_path", string("/tmp/image.png"));
  node->declare_parameter("bag_path", string("/tmp/input"));
  node->declare_parameter("lidar_topic", string("/livox/lidar"));
  node->declare_parameter("output_path", string("/tmp/output"));
  node->declare_parameter("use_auto_lidar_roi", false);
  ...
  node->declare_parameter("lidar_flip_yz", false);
  ...
  params.annulus_half_width = node->get_parameter("annulus_half_width").as_double();
  params.board_width = node->get_parameter("board_width").as_double();
  params.board_height = node->get_parameter("board_height").as_double();
  params.board_roi_margin = node->get_parameter("board_roi_margin").as_double();
  params.board_roi_depth = node->get_parameter("board_roi_depth").as_double();
  params.auto_roi_voxel_leaf = node->get_parameter("auto_roi_voxel_leaf").as_double();
  params.annulus_voxel_leaf = node->get_parameter("annulus_voxel_leaf").as_double();
  ...
  params.use_auto_lidar_roi = node->get_parameter("use_auto_lidar_roi").as_bool();
  ...
  params.lidar_flip_yz = node->get_parameter("lidar_flip_yz").as_bool();
  return params;
}
...
inline bool ensureOutputDir(const std::string& output_path)
{
                      ...
                      const Params& params)
{
    ...
    if (!ensureOutputDir(saveDir)) {
        std::cerr << "[saveTargetHoleCenters] Cannot create output dir: " << saveDir << std::endl;
        return;
    }
     ...
     const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& colored_cloud, const cv::Mat& img_input)
{
  ...
  if (!ensureOutputDir(outputDir)) {
    std::cerr << BOLDRED << "[saveCalibrationResults] Cannot create output dir: " << outputDir << RESET << std::endl;
    return;
  }
                        ...
                        bool lidar_flip_yz = false)
{
  ...
  if (axis_mode == "lidar") {
    for (auto& point : v->points) {
      if (lidar_flip_yz) {
        const float x_new = point.z;
        const float y_new = point.x;
        const float z_new = point.y;
        point.x = x_new;
        point.y = y_new;
        point.z = z_new;
      } else {
...
void canonicalizePatternOrder(pcl::PointCloud<pcl::PointXYZ>::Ptr v,
                              double width,
                              double height)
{
  if (v->size() != 4) return;
  ...
  auto edge = [&](int a, int b) {
    const auto& p1 = v->points[a];
    const auto& p2 = v->points[b];
  ...
  for (int k = 0; k < 4; ++k) {
    const double e01 = edge(k, (k + 1) % 4);
    const double e12 = edge((k + 1) % 4, (k + 2) % 4);
    const double err = std::abs(e01 - width) / width + std::abs(e12 - height) / height;
    if (err < best_err) {
      best_err = err;
      best_k = k;
    }
  ...
  rotated.resize(4);
  for (int i = 0; i < 4; ++i) {
    rotated.points[i] = v->points[(i + best_k) % 4];
  }
  *v = rotated;
}
